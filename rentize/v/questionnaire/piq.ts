...
export class register_intern
    ...
    get_label(input: HTMLInputElement): quest.label {
        ...
        const database:string= Array.from(inputs).map(input=> (<HTMLInputElement> input).data-dbname);
        //
        //Get the entity name
        const ename:string= Array.from(inputs).map(input=> (<HTMLInputElement> input).data-ename);
        //
        //Get the alias
        const alias:Array<basic_value>= Array.from(inputs).map(input=> (<HTMLInputElement> input).alias);
        //
        //Get the column name
        const cname:string= Array.from(inputs).map(input=> (<HTMLInputElement> input).name);
        //
        //Get the expression value
        const expression:Array<basic_value>= Array.from(inputs).map(input=> (<HTMLInputElement> input).expression);
        //
        //Construct and return the label
        return label =[database, ename, alias, cname, expression];
    }
