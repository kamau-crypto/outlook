...
type t_delivery = {
    merchant: string;
    reference: string;
    description: Array<[string, number, number]>;
}
//
...
class event extends popup<Ievent>{
    ...
    async get_results(): Promise<Ievent>{
        //
        //Get the reference number.
        const event = this.get_element("organization");
        //
        if (!(event instanceof HTMLInputElement)){
            throw new Error(`Element of 'organization' is not a valid textarea element.`);
        }
        //
        //Compile the event.
        const org_value: Ievent = {event: event.value}
        //
        return org_value;
    }
    //
...
class msg extends popup<Imsg> {
    ...
    async get_result(): Promise<Imsg> {
        //
        //Get the message.
        const msg = this.get_element("msg");
        //
        if (!(msg instanceof HTMLTextAreaElement)){
            throw new Error(`Element of 'msg' is not a valid textarea element.`);
        }
        //
        //Compile the message.
