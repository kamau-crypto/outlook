    ...
    function activate_class($classname, $obj) {//mutall
        ...
        switch ($classname) {
            ...
            case "record":
                ...
                //record anyway.
                $class = new record($fields, $dbase, $obj->tname, $obj->reftable, $obj->sql, $obj->values);
                break;
            //      
...
class page extends querystring {
    ...
    //layout
    function display_layout_header(sql $sql, layout $layout) {
        //
    ...
    function display_header_fields(sql $sql) {//page
        //
    ...
    function display_record(record $record, $layout = null, $mode = null) {//page
        //
...
abstract class page_table extends page {
    ...
    function save_current_record($jvalues = null) {//page_table
        //
...
class column_relation extends column {
    ...
    function split() {//column_relation
        //
    ...
    function set_value() {//column_relatiom
        //
        //A relation column has 3 subfields; step through each one of them
        foreach (field::map as $key => $value) {
            //
...
class column_primary extends column_relation {
    ...
    function initialize_edit_sql($sql_edit) {//column_primary
        ...
        $this->subfields->{$this->tname} = new field($this->tname, $xvalue);
        //
...
class column_foreign extends column_relation {
    ...
    function initialize_edit_sql($sql_edit) {//column_foreign
        ...
        foreach (field::map as $index => $fname) {
            ...
            $cfield = new field($cfname, $cxvalue);
            //
    ...
    function initialize_hint_sql($sqlHintPrimary) {
        ...
        foreach ($sqlHintSecondary->fields as $index => $field) {
            ...
            $name = $field->name;
            //        
            $sqlHintPrimary->fields[$index] = new field($name, $xvalue);
        }
        //
    ...
    function field_value() {//column_foreign
        //
...
//
//A special database that is Buis needs to support its operation
...
class sql_edit extends sql {
    ...
    function get_record(stdClass $values = null, $metadata=null) {
        ...
        $record = new record($this->fields, $this->dbase, $this->tname, $this->reftable, $this->sql, $values);
        //
...
class sql_selector extends sql implements table {
    ...
    function initialize_sql_data() {//sql_selector
        ...
        $this->fields[field::primary] = new field(field::primary, $primaryfield->xvalue);
        //
...
//The page home class is used for extending websites
class page_home extends page {
    ...
    function __construct(querystring $qstring, $website = null, $dbname = null) {
        ...
        //
        //Set the database for the home page
        $this->dbase = new dbase(page_login::password, page_login::username, $dbname);
        //
    ...
    function get_driver() {
        //
        return new xml("../$this->website/$this->website.xml", $this->dbname);
    }
...
//own is to allow other pages, e.g., page_buis, to access the login/out 
...
class page_records extends page_table {
    ...
    function get_driver() {//page_records
        ...
        $driver = new sql($sql_edit->dbase, $stmt, $fields);
        //
        return $driver;
    }
...
class page_record extends page_table {
    ...
    function activate_class($classname, $obj) {//page_record
        //
...
class page_selector extends page_records {
    ...
    function search_hint($hint = null) {
        ...
        $stmt2 = "select stmt1.* from ($driver1->sql) as stmt1 where " . field::output . " like '%" . $hint . "%'";
        //
